...
def get_args():
    psr = ArgumentParser()
    psr.add_argument("--curr-rating", type=float, required=True, help="Your current rating.") # TODO: handle provisional
...
def validate(args):
    ...
    if args.curr_rating < USCF_FLOOR:
...
def find_performance_rating(init, opps, score, tol=1e-3):
    """
        ...
        -- note -- doesn't work for perfect/imperfect scores
    """
    if score == 0:
        return min(opps) - 400
    if score == len(opps):
...
def df_to_table(df):
    ...
    """
    rich_table = Table()
    rich_table.add_column(df.index.name)
...
def print_results(performance, score, we, curr_rating, new_rating, norms_table):
    ...
    print("[bold]Performance:[/bold]", performance)
...
if __name__ == '__main__':
    ...
    print_results(tpr, args.score, we, args.curr_rating, new_rating, norms_table)
